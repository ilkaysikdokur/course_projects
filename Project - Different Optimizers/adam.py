...
#adam
epsilon = 0.00000001
mW1 = np.array([[0 for k in range(K)] for l in range(784)])
vW1 = np.array([[0 for k in range(K)] for l in range(784)])
...
while i<N: #i<len(x_train): 
    ...
    for m in range(len(v_W1)):
        for n in range(len(v_W1[m])):
            v_W1[m][n] = eta/(epsilon + np.sqrt(v_W1[m][n]))
    ...
    for m in range(len(v_W2)):
        for n in range(len(v_W2[m])):
            v_W2[m][n] = eta/(epsilon + np.sqrt(v_W2[m][n]))
