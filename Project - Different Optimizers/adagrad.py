...
#adagrad
epsilon = 0.00000001
mtrxW1 = [[0 for k in range(K)] for l in range(784)]
mtrxW2 = [[0 for k in range(10)] for l in range(K)]
...
while i<N: #i<len(x_train): 
    ...
    for m in range(len(mtrxW1)):
        for n in range(len(mtrxW1[m])):
            mtrxW1[m][n] = 1/np.sqrt(epsilon + dL_dW1[m][n]*dL_dW1[m][n])

    mtrxW1 = np.interp(mtrxW1, [np.min(mtrxW1), np.max(mtrxW1)], [0, 1])
    
    
    W1new = np.array(tf.math.subtract(W1, tf.math.multiply(mtrxW1, eta*dL_dW1)), np.double)
    ...
    for m in range(len(mtrxW2)):
        for n in range(len(mtrxW2[m])):
            mtrxW2[m][n] = 1/np.sqrt(epsilon + dL_dW2[m][n]*dL_dW2[m][n])
    
    mtrxW2 = np.interp(mtrxW2, [np.min(mtrxW2), np.max(mtrxW2)], [0, 1])
    
    W2new = np.array(tf.math.subtract(W2, tf.math.multiply(mtrxW2, eta*dL_dW2)), np.double)
    #W2new = np.interp(W2new, [np.min(W2new), np.max(W2new)], [-1,1])
    #W2new = np.array(tf.math.sigmoid(W2new))
    
    diffArr = np.array(tf.math.subtract(outActual[0], outCalculated[0]), np.double)
    diff = 0
    for k in range(10):
