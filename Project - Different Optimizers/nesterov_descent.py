...
while i<N: #i<len(x_train): 
    ...
    rep=0
    while L>0.01:
        eta1 = random.uniform(10**-10, 10**-3)
        eta2 = random.uniform(10**-10, 10**-3)
        gamma = random.uniform(0.0001, 0.9999)
...
while j<N/6:#j<len(x_test):
    ...
    if tf.keras.backend.eval(tf.math.argmax(h2[0])) == y_test[j]:
        true += 1
        print(str(j+1)+"/"+str(len(x_test))+": Dogru")
    else:
        false += 1
        print(str(j+1)+"/"+str(len(x_test))+": Yanlis")
