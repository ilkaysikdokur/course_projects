...
for e in range(epoch):
    ...
    for i in randomNArr:
        ...
        for r in range(imageX):
            for c in range(imageY):  
                v_raw[r+padding][c+padding] = x_train[i][r][c]
        
        for k in range(featureMapX):
            for l in range(featureMapY):
                ftrVal = 0
        ...
        vW1 = beta2*vW1 + (1-beta2)*dL_dW1*dL_dW1
        ...
        vW2 = beta2*vW2 + (1-beta2)*dL_dW2*dL_dW2
    ...
    if e+1 in [1, 2, 3, 4, 5, 10, 15, 20, 30]:
