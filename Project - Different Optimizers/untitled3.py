...
#    for c in range(K):
        ...
        #W1.append(0.5)
#        W1.append(random.uniform(r*c-1, r*c) * np.sqrt(2/(r*c)))
W1 = np.random.rand(poolMapLength,K)*np.sqrt(1/(poolMapLength+10))
...
#        #W2.append(np.random.standard_normal(1))
        #W2.append(0.5)
        #W2.append(0)
#        W2.append(random.uniform(r*c-1, r*c) * np.sqrt(2/(r*c)))
W2 = np.random.rand(K,10)*np.sqrt(1/(poolMapLength+10))
...
dL_dout = [[0 for k in range(10)]]
dsoftmax = [[0 for k in range(10)] for l in range(10)]
dh2_dh1 = []
dh1_dW1 = [[0 for k in range(1)] for l in range(poolMapLength)]
dh2_dW2 = [[0 for k in range(1)] for l in range(K)]
...
for e in range(epoch):
    ...
    if count % batchSize == 0:
        dL_dout = np.array([[0 for k in range(10)]], np.double)
        dsoftmax = np.array([[0 for k in range(10)] for l in range(10)], np.double)
        dh1_dW1 = np.array([[0 for k in range(1)] for l in range(poolMapLength)], np.double)
        dh2_dW2 = np.array([[0 for k in range(1)] for l in range(K)], np.double)
    ...
    for i in randomNArr:
        ...
        v = np.array(tf.nn.relu(v), np.double)
        for k in range(len(v[0])):
            if random.uniform(0.0,1.0)<dropoutInput:
                v[0][k]=0   
        ...
        #   if random.uniform(0.0,1.0)>dropoutRate:
        #       h1[0][k] = 0
        #h1 = np.array(tf.nn.tanh(h1), np.double)
        h1 = np.array(tf.nn.leaky_relu(h1), np.double)
        for k in range(len(h1[0])):
            if random.uniform(0.0,1.0)<dropoutInput:
                h1[0][k]=0   
        ...
        #dL/dout
        dL_dout += [[((outActual[0][k]/outCalculated[0][k]) - ((1-outActual[0][k])/(1-outCalculated[0][k])))*-1/(10*np.log(2)) for k in range(10)]]
        ...
        dL_dW1 = m_W1*eta/(epsilon + np.sqrt(v_W1))
        dL_dW2 = m_W2*eta/(epsilon + np.sqrt(v_W2))
        W1 -= dL_dW1
        #W1 = piecewiseLinearFunction(W1)
        W2 -= dL_dW2
        #W2 = piecewiseLinearFunction(W2)
